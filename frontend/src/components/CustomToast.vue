...
<template>
  <div class="fixed bottom-4 right-4 left-4 sm:left-auto sm:top-6 sm:bottom-auto sm:right-6 z-[9999] flex flex-col gap-3 sm:gap-4 pointer-events-none items-center sm:items-end">
    <TransitionGroup 
    ...
    >
      <div 
        ...
        :key="toast.id"
        :class="['pointer-events-auto flex items-start p-3 sm:p-4 rounded-xl sm:rounded-2xl w-full max-w-[90vw] sm:min-w-[320px] sm:max-w-sm relative overflow-hidden group', getContainerClass(toast.type)]"
      >
        <!-- Icon -->
        <div :class="['w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 mr-3 sm:mr-4', getIconBoxClass(toast.type)]">
          <component :is="getIcon(toast.type)" class="w-4 h-4 sm:w-5 sm:h-5" />
        </div>
        
        <!-- Content -->
        <div class="flex-grow pt-0 sm:pt-0.5">
          <h4 class="text-xs sm:text-sm font-bold text-slate-900 leading-tight mb-0.5 sm:mb-1">
            {{ toast.type === 'success' ? 'Success!' : toast.type === 'error' ? 'Oops!' : 'Notification' }}
          </h4>
          <p class="font-medium text-[10px] sm:text-xs text-slate-500 leading-relaxed">{{ toast.message }}</p>
        </div>
        
        <!-- Close Button -->
        <button @click="ui.removeToast(toast.id)" class="ml-2 sm:ml-4 flex-shrink-0 p-1 sm:p-1.5 rounded-md sm:rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors focus:outline-none">

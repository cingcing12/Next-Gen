...
const handleRegister = async () => {
  ...
  if (!agreeTerms.value) {
    ui.toast('Please agree to the Terms of Service to continue.', 'error')
    return
  }
  ...
  try {
    ...
    ui.toast(err.response?.data?.message || 'Registration failed', 'error')
  } finally {
...
const handleGoogleCredential = async (response) => {
  try {
    ...
    ui.toast('Account created with Google! Welcome 🎉', 'success')
    router.push('/')
  } catch (err) {
    ui.toast(err.response?.data?.message || 'Google sign-up failed', 'error')
  }
}
...
<template>
  <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-hidden">
    ...
    <div class="absolute -top-32 -right-32 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl pointer-events-none"></div>
    ...
    <div class="w-full max-w-5xl bg-white rounded-3xl shadow-2xl shadow-slate-200/60 border border-slate-100 overflow-hidden grid grid-cols-1 lg:grid-cols-12 relative z-10">
      ...
      <div class="hidden lg:flex lg:col-span-5 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-white p-10 flex-col justify-between relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-purple-500/20 text-purple-300 border border-purple-500/30 mb-4">
            <Gift class="w-3.5 h-3.5 text-purple-400" />
            <span>Join Next-Gen Club</span>
          </div>

          <h2 class="text-3xl font-extrabold tracking-tight leading-snug">
            Create your account and unlock exclusive member perks.
          </h2>
          <p class="text-slate-400 text-sm mt-3 leading-relaxed">
            Get early access to streetwear drops, automatic order tracking, and express Bakong checkout.
          </p>
        </div>
        ...
        <div class="relative z-10 space-y-3.5 my-8">
          <div class="flex items-center gap-3 text-xs font-semibold text-slate-300">
            ...
            <span>Instant KHQR QR payments with NBC Bakong</span>
          </div>
          <div class="flex items-center gap-3 text-xs font-semibold text-slate-300">
            <div class="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center">
              <CheckCircle2 class="w-3.5 h-3.5" />
            </div>
            <span>Save favorite items to your personal Wishlist</span>
          </div>
          <div class="flex items-center gap-3 text-xs font-semibold text-slate-300">
            <div class="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center">
              <CheckCircle2 class="w-3.5 h-3.5" />
            </div>
            <span>Write reviews and earn verified buyer badges</span>
          </div>
          <div class="flex items-center gap-3 text-xs font-semibold text-slate-300">
            <div class="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center">
              <CheckCircle2 class="w-3.5 h-3.5" />
            </div>
            <span>Live delivery status with Vireak Buntham Express</span>
          </div>
        </div>

        <!-- Special Welcome Offer Pill -->
        <div class="relative z-10 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-purple-500/20 text-purple-400 flex items-center justify-center flex-shrink-0">
            <Tag class="w-5 h-5" />
          </div>
          <div>
            <p class="text-xs font-bold text-white">Free Express Shipping</p>
            <p class="text-[11px] text-slate-400">On all orders over $100 across Cambodia</p>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-7 p-6 sm:p-10 md:p-12 flex flex-col justify-between">
        ...
        <div>
          ...
          <div class="lg:hidden flex items-center justify-between mb-6">
            ...
            <router-link to="/login" class="text-xs font-bold text-indigo-600 hover:underline">
              Sign in →
            </router-link>
          </div>
          ...
          <div class="mb-6 sm:mb-8">
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-1">
              Start shopping premium streetwear in seconds.
            </p>
          </div>
          ...
          <!-- Sign Up Form -->
          <form class="space-y-3.5 sm:space-y-4" @submit.prevent="handleRegister">
            ...
            <!-- Password Input -->
            <div>
              <label for="password" class="block text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-600 mb-1 sm:mb-1.5">
                Password
              </label>
              <div class="relative rounded-lg sm:rounded-xl border border-slate-200 focus-within:border-indigo-600 focus-within:ring-2 focus-within:ring-indigo-600/20 transition-all bg-slate-50/60 focus-within:bg-white">
                <div class="absolute inset-y-0 left-0 pl-3 sm:pl-3.5 flex items-center pointer-events-none text-slate-400">
            ...
            <div class="flex items-start pt-1">
              <input
                ...
                type="checkbox"
                required
                class="w-3.5 h-3.5 sm:w-4 sm:h-4 mt-0.5 sm:mt-1 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer"
              />
              <label for="agree-terms" class="ml-2 block text-[10px] sm:text-xs text-slate-600 cursor-pointer leading-tight">
                I agree to the <a href="#" class="text-indigo-600 font-semibold hover:underline">Terms of Service</a> and <a href="#" class="text-indigo-600 font-semibold hover:underline">Privacy Policy</a>
              </label>
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 text-center">
          <p class="text-sm text-slate-600">
            ...
            <router-link to="/login" class="font-bold text-indigo-600 hover:text-indigo-700 ml-1">

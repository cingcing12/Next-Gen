    ...
    <div v-else-if="product">
      ...
      <div id="reviews" v-if="product" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 border-t border-slate-200 mt-10 sm:mt-16">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          <div class="lg:col-span-5 bg-gradient-to-br from-slate-50 via-white to-amber-50/20 p-7 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
            <div>
              <h3 class="text-sm font-semibold uppercase tracking-wider text-slate-500 mb-4">Rating Breakdown</h3>
              
              <div class="flex items-center gap-6 mb-6">
                <div class="text-center">
                  <div class="text-5xl font-black text-slate-900 tracking-tight leading-none">{{ averageRating }}</div>
                  ...
                  <p class="text-xs text-slate-500 mt-1 font-medium">out of 5.0</p>
                </div>

                <div class="h-16 w-px bg-slate-200"></div>

                <div>
                  <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-bold mb-1.5 border border-emerald-100">
              ...
              <div class="space-y-2.5">
                <div v-for="item in reviewBreakdown" :key="item.stars" class="flex items-center gap-3 text-xs">
                  <div class="flex items-center gap-1 w-12 text-slate-600 font-medium">
                  ...
                  <div class="w-12 text-right text-slate-400 font-mono font-medium">
                    {{ item.percentage }}%
                  </div>
                </div>
              </div>
            ...
            <div class="mt-6 pt-5 border-t border-slate-200/60 flex items-center justify-between text-xs text-slate-500">
              <span class="flex items-center gap-1.5">
                <Check class="w-4 h-4 text-emerald-500" /> 100% Verified purchases
              </span>
              <span class="flex items-center gap-1.5">
                <Award class="w-4 h-4 text-indigo-500" /> Quality assured
              </span>
            </div>
          </div>

          <!-- Write Review Card (7 cols) -->
          <div class="lg:col-span-7 bg-white p-7 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
            <div>
              <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-2.5">
                  <div class="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold">
                    <MessageSquare class="w-5 h-5" />
                  </div>
                  <div>
                    <h3 class="text-base font-bold text-slate-900">Write a Review</h3>
                    <p class="text-xs text-slate-500">Share your thoughts with the community</p>
                  </div>
                </div>
                <span v-if="authStore.isAuthenticated" class="text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full font-semibold flex items-center gap-1">
                  <UserCheck class="w-3.5 h-3.5" /> Logged In
                </span>
              </div>

              <!-- Unauthenticated State -->
              <div v-if="!authStore.isAuthenticated" class="text-center py-8 px-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                <div class="w-12 h-12 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center mx-auto mb-3">
                  <UserCheck class="w-6 h-6" />
                </div>
                <h4 class="text-sm font-bold text-slate-800 mb-1">Have you tried this product?</h4>
                ...
                <router-link
                  to="/login"
                  class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-black text-white text-xs font-bold rounded-xl transition shadow-sm hover:shadow"
                >
                  Log In to Review
                </router-link>
              </div>
              ...
              <form v-else @submit.prevent="submitReview" class="space-y-4">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <div class="flex items-center gap-1.5 p-2 bg-slate-50 rounded-xl border border-slate-200/80">
                      <button
                        v-for="star in 5"
                        :key="star"
                        type="button"
                        ...
                        class="p-1 rounded-lg transition-transform transform hover:scale-125 focus:outline-none"
                      >
                        <Star
                    ...
                    <span class="px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200/60">
                ...
                <div>
                  <div class="flex items-center justify-between mb-1.5">
                    ...
                    <span class="text-xs text-slate-400 font-mono">{{ reviewComment.length }} chars</span>
                  </div>
                  <textarea
                    ...
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition resize-none"
                  ></textarea>
                </div>
                ...
                <div class="flex items-center justify-between pt-1">
                  ...
                  <button
                    ...
                    class="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-bold text-xs px-6 py-3 rounded-xl transition-all shadow-md shadow-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                  >
        ...
        <div>
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-bold text-slate-900 flex items-center gap-2">
              ...
              <span class="text-xs font-semibold px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md">{{ product.reviews?.length || 0 }}</span>
            </h3>
          </div>

          <!-- Empty Reviews State -->
          <div v-if="!product.reviews || product.reviews.length === 0" class="text-center py-16 px-4 bg-slate-50/70 border border-dashed border-slate-200 rounded-3xl">
            <div class="w-14 h-14 bg-white shadow-sm border border-slate-100 rounded-2xl flex items-center justify-center mx-auto mb-3 text-slate-400">
              <MessageSquare class="w-7 h-7" />
            </div>
            <h4 class="text-base font-bold text-slate-800 mb-1">No reviews yet</h4>
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div
              ...
              :key="review._id"
              class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between"
            >
              <div>
                <!-- Reviewer Header -->
                <div class="flex items-start justify-between gap-3 mb-3">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-violet-500 text-white font-bold text-sm flex items-center justify-center shadow-sm uppercase">
                      {{ (review.name || 'U').charAt(0) }}
                    </div>
                    <div>
                      <div class="flex items-center gap-2">
                        <h4 class="text-sm font-bold text-slate-900">{{ review.name || 'Customer' }}</h4>
                      ...
                      <div class="flex items-center gap-2 mt-0.5">
                        ...
                        <span class="text-[11px] font-semibold text-slate-500">{{ ratingLabels[review.rating] || `${review.rating} Stars` }}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                ...
                <button
                  ...
                  class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg border transition-colors cursor-pointer"
                ...
                >
                  <ThumbsUp class="w-3.5 h-3.5" :class="helpfulReviews[review._id] ? 'fill-indigo-600' : ''" />
